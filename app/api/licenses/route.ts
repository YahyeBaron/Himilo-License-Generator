...
export async function POST(req: Request) {
    try {
        const { clientName, companyId, expiresAt, maxDevices, status } = await req.json()
        if (!clientName) {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
            [id, companyId || null, clientName, licenseKey, status || 'active', expiresAt || null, maxDevices || 1]
        )
